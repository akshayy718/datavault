"use client";
/**
 * RecipientClient — handles PIN unlock and renders the share data.
 *
 * This component receives pre-fetched data from the server (page.tsx).
 * If the share is PIN-protected (status 401), it shows the PIN gate.
 * The phone only makes a network request for PIN verification — everything
 * else was already fetched server-side and arrives as props.
 */
...
export default function RecipientClient({ token, initialData, initialStatus, backendOrigin }: Props) {
  const [data, setData] = useState<ViewData | null>(initialData);
  const [locked, setLocked] = useState(initialStatus === 401);
  const [errorMsg, setErrorMsg] = useState<string | null>(
    initialStatus === 410 ? "This share is no longer available (expired, revoked, or view limit reached)." :
    initialStatus === 404 ? "This link doesn't exist." :
    initialStatus === 503 ? "Server is temporarily unavailable. Please try again in a moment." :
    initialStatus !== 200 && initialStatus !== 401 ? "Something went wrong loading this share." :
    null
  );
  ...
  async function handleUnlock() {
    ...
    if (pin.length === 0) { setPinError("Please enter your PIN."); return; }
    setPinError("");
    setUnlocking(true);
      ...
      <AnimatePresence mode="wait">
            ...
            <div className="w-full max-w-sm text-center">
              ...
              {initialStatus === 503 && (
                <button
                  ...
                  className="mt-5 rounded-xl bg-[#00E6A7] px-5 py-2.5 text-sm font-medium text-[#08090A]"
                >
                  Try again
                </button>
              )}
        ...
        {!locked && !errorMsg && data && (
          ...
          >
            ...
            >
              {kind === "single" && fields && <SingleCard fields={fields} mode={data.mode} />}
              {kind === "multi" && items && <MultiCard items={items} mode={data.mode} />}
